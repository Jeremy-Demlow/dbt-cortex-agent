...
Make one schema-versioned dbt macro payload authoritative for evaluation identity, native config, metric policy, dataset/stage context, and deterministic suite signature inputs.
...
Evaluation evidence is reproducibly tied to one dbt-validated execution plan and one stable Agent version, while Python remains a transport, polling, artifact, and comparison client rather than a second metadata renderer.
...
4. Python obtains and validates the plan through `dbt run-operation` after fresh parse; injectable plan/command fakes remain supported.
