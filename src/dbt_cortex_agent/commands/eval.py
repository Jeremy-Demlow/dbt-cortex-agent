...
def register(subparsers: argparse._SubParsersAction, shared: argparse.ArgumentParser) -> None:
    parser = subparsers.add_parser(
        ...
        description=(
            "Plan evaluations locally, opt into paid runs, and manage local comparison evidence."
        ),
        epilog=(
            "Examples:\n"
    ...
    run = commands.add_parser(
        "run", parents=[shared], help="plan or execute evaluation [PAID with --apply]"
    )
    ...
    run.add_argument(
        "--poll-interval", type=float, default=30, help="seconds between polls (default: 30)"
    )
    run.add_argument(
        "--transient-retries",
        type=int,
        default=1,
        help="retries for transient failures (default: 1)",
    )
    add_allowlists(run)
    run.add_argument(
        "--apply",
        action="store_true",
        help="[PAID] start evaluation; default only renders the plan",
    )
    ...
    verify = commands.add_parser(
        "verify",
        parents=[shared],
        help="materialize, run, and gate evaluations [PAID with --apply]",
    )
    ...
    verify.add_argument(
        "--apply",
        action="store_true",
        help="[PAID] materialize and execute evaluations; default is preview",
    )
    verify.set_defaults(handler=handle)

    compare = commands.add_parser(
        "compare", parents=[shared], help="compare baseline and candidate artifacts"
    )
    ...
    accept = commands.add_parser(
        "accept-baseline",
        parents=[shared],
        help="preview or accept a passing baseline [MUTATION with --apply]",
    )
    accept.add_argument("candidate")
    accept.add_argument("--baseline-dir")
    accept.add_argument(
        "--apply", action="store_true", help="[MUTATION] write the baseline; default is preview"
    )
    accept.add_argument(
        "--force", action="store_true", help="overwrite an existing baseline when applying"
    )
    accept.set_defaults(handler=handle)

    gate = commands.add_parser(
        "gate", parents=[shared], help="gate a candidate against thresholds and baseline"
    )
...
def _handle_verify(args: argparse.Namespace, config: Config) -> int:
    if args.apply:
...
def _handle_run(args: argparse.Namespace, config: Config) -> int:
    if args.apply:
        require_explicit_connection(config, "Evaluation apply")
    plan = build_plan(config, agent_name=args.agent, suite_name=args.suite, parse=not args.no_parse)
    plan_payload = {
    ...
    payload = {
        "command": "eval run",
        "plan": plan_payload,
        "candidate": str(output) if output else None,
        "passed": candidate.get("passed") if candidate else None,
    }
    if args.json:
        emit_json(payload)
    else:
...
def _handle_compare(args: argparse.Namespace, _config: Config) -> int:
    result = compare_results(
        load_result(args.baseline), load_result(args.candidate), args.tolerance
    )
    emit_json(result)
    return 0 if result["passed"] else 1


def _handle_accept(args: argparse.Namespace, config: Config) -> int:
    ...
    target = accept_baseline(candidate, directory, force=args.force) if args.apply else None
    payload = {
...
def _handle_gate(args: argparse.Namespace, config: Config) -> int:
    directory = args.baseline_dir or str(config.artifact_dir / "baselines")
...
def handle(args: argparse.Namespace, config: Config) -> int:
    handlers = {
        ...
        "accept-baseline": _handle_accept,
        "gate": _handle_gate,
    }
    return handlers[args.eval_command](args, config)
